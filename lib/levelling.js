...
export function xpRange(level, multiplier = globalThis.multiplier || 1) {
    if (level < 0)
...
export function findLevel(xp, multiplier = globalThis.multiplier || 1) {
    if (xp === Infinity)
        return Infinity
    if (isNaN(xp))
        return NaN
...
export function canLevelUp(level, xp, multiplier = globalThis.multiplier || 1) {
    if (level < 0)
        return false
    if (xp === Infinity)
        return true
