...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    let str;
    if (m.mentionedJid.length > 0) {
        str = `${name2} le dió una rusa a ${name || who}.`; // Usamos nombre agendado o número si no está agendado
    } else if (m.quoted) {
        str = `${name2} le está haciendo una rusa a ${name || who}.`; // Mensaje cuando se cita a otro usuario
    } else {
        str = `${name2} está dando una rusa`.trim();
    }
