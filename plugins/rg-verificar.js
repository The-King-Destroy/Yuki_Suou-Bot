...
let handler = async function (m, { conn, text, usedPrefix, command }) {
  ...
  let who2 = m.isGroup ? (m.mentionedJid && m.mentionedJid.length > 0 ? m.mentionedJid[0] : (m.quoted?.sender || m.sender)) : m.sender;
  ...
  if (user.registered === true) {
    throw `*『✦』Ya estas registrado, para volver a registrarte, usa el comando: #unreg*`;
  }
