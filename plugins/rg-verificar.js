...
let handler = async function (m, { conn, text, usedPrefix, command }) {
    ...
    if (!biografia || !biografia[0] || biografia[0].status === null) {
        ...
        fechaBio = biografia[0].setAt ? new Date(biografia[0].setAt).toLocaleDateString("es-ES") : "Fecha no disponible";
    }

    if (user.registered) {
        throw `*『✦』Ya estás registrado, para volver a registrarte, usa el comando: #unreg*`;
    }
