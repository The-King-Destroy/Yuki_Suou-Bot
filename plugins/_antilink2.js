...
export async function before(m, {isAdmin, isBotAdmin, text}) {
  ...
  if (chat.antiLink2 && isGroupLink && !isAdmin) {
    ...
    if (!isBotAdmin) return m.reply(`${emoji2} No soy admin!! por lo tanto no puedo ejecutar la acción de expulsión.`);
    ...
    } else if (!bot.restrict) return m.reply(`${emoji} El owner no tiene activa la opción de restringir, no puedo ejecutar está acción.`);
  }
  return !0;
}