...
const handler = async (m, { conn, text, command, args, usedPrefix }) => {
    if (!text) return conn.reply(m.chat, '✧ Te faltó el texto para hablar con Yuki Suou.', m);
    
    try {
        const resSimi = await simitalk(text);
        const yukiResponse = formatResponse(resSimi.resultado.simsimi, text);
        conn.sendMessage(m.chat, { text: yukiResponse }, { quoted: m });
    } catch {
