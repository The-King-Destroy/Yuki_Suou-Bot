...
const giveCharHandler = async (m, { conn, args, mentionedJid }) => {
    const groupId = m.chat;
    const userId = m.sender;
    const harem = await loadHarem();

    if (!args[0] || !mentionedJid[0]) {
        await conn.reply(m.chat, 'Debes mencionar a un usuario y el nombre del personaje.', m);
        return;
    }
    ...
    await conn.reply(m.chat, `✦ Has regalado con éxito *${character.name}* a *@${targetUserId.split('@')[0]}*!`, m);
};

giveCharHandler.help = ['givechar', 'regalar'];
giveCharHandler.tags = ['gacha'];
giveCharHandler.command = /^(givechar|regalar)$/i;
