...
export async function before(m, { conn, participants, groupMetadata }) {
    ...
    if (chat.detect && m.messageStubType == 21) {
        ...
        let isAdmin = participants.find(participant => participant.id === m.sender && participant.admin);
        
        if (isAdmin) {
            let nuevoParticipante = m.messageStubParameters[0];
            let mensajeAlerta = `🍬 Un nuevo participante ha ingresado al grupo:\n\n*◦ 🍭 Usuario:* @${nuevoParticipante.split('@')[0]}\n*◦ 🍭 Aceptado por:* @${m.sender.split('@')[0]}`;

            await conn.sendMessage(m.chat, { text: mensajeAlerta, mentions: [nuevoParticipante, m.sender] }, { quoted: fkontak });
        }
