...
if (!db.data.chats[m.chat].nsfw && m.isGroup) return m.reply('🚩 *¡Estos comandos están desactivados!*');
    ...
    let str;
    if (m.mentionedJid.length > 0) {
        str = `${name2} está manoseando a ${name || who}.`; // Usamos nombre agendado o número si no está agendado
    } else if (m.quoted) {
        str = `${name2} le está manoseando a ${name || who}.`; // Mensaje cuando se cita a otro usuario
    } else {
