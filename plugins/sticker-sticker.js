...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    try {
        ...
        if (/webp|image|video/g.test(mime)) {
            ...
            if (!media) return conn.reply(m.chat, `${emoji} Por favor, envia una imagen o video para hacer un sticker.`, m)

            let out
            try {
                if (/image/.test(mime)) {
                    let crop = /\-x|\-crop/i.test(m.text);
                    stiker = await imageToWebp(media, crop, { packsticker: global.packsticker, packsticker2: global.packsticker2 });
